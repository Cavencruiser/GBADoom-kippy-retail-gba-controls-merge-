...
{
    // CPhipps - patch drawing updated
    V_DrawNamePatch(54, 38, 0, "M_EPISOD");
}
...
{
    // CPhipps - patch drawing updated
    V_DrawNamePatch(96, 14, 0, "M_NEWG");
    V_DrawNamePatch(54, 38, 0, "M_SKILL");
}
...
void M_ChooseSkill(int choice)
{
    ...
    {
        G_DeferedInitNew(choice,_g->epi+1,9);
        M_ClearMenus ();
    }
}
...
void M_DrawLoad(void)
{
    ...
    // CPhipps - patch drawing updated
    V_DrawNamePatch(72 ,LOADGRAPHIC_Y, 0, "M_LOADG");
...
void M_DrawSave(void)
{
    ...
    //jff 3/15/98 use symbolic load position
    // CPhipps - patch drawing updated
    V_DrawNamePatch(72, LOADGRAPHIC_Y, 0, "M_SAVEG");
...
{
    // CPhipps - patch drawing updated
    V_DrawNamePatch(60, 38, 0, "M_SVOL");
...
void M_Drawer (void)
{
    ...
    if (_g->messageToPrint)
    {
        ...
        if (_g->menuactive)
        {
            ...
            // CPhipps - patch drawing updated
            V_DrawNamePatch(x + SKULLXOFF, _g->currentMenu->y - 5 + _g->itemOn*LINEHEIGHT,0,skullName[_g->whichSkull]);
        }
}
...
void M_DrawThermo(int x,int y,int thermWidth,int thermDot )
{
   ...
   */
    ...
    V_DrawNamePatch(xx, y, 0, "M_THERML");

    xx += 8;
    for (i=0;i<thermWidth;i++)
    {
        V_DrawNumPatch(xx, y, 0, thermm_lump);
        xx += horizScaler;
    }

    xx += (8 - horizScaler);  /* make the right end look even */

    V_DrawNamePatch(xx, y, 0, "M_THERMR");
